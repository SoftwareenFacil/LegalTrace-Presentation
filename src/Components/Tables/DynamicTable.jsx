...
const DynamicTable = ({
  ...
  const renderTableRows = (data, attributes, category) =>
    data.map((item, index) => {
      const rowClass =
        category === "tasks" && item.vigency ? null : "disable-color-table";
      ...
      return (
        <tr key={mainKey}>
          {category === "tasks" ? (
            <td
              key={`${mainKey}-icon`}
              className={`${!item.vigency ? rowClass + " icon-div" : ""}`}
            >
              {renderIcon(
                item.vigency ? item.finished : undefined,
          ...
          {attributes.map((attr) => {
            ...
            if (
              category &&
              (attr.key === "vigency" || attr.key === "finished")
            ) {
              return (
                <td
                  key={cellKey}
                  className={category === "tasks" ? rowClass : ""}
                >
                  <BadgeVigency
            ...
            // }
              ...
              return (
                <td key={cellKey}>
                  {item.type}
                  {/* cambiar-fijos: F29, Renta, Leyes Sociales, Otros */}
                </td>
              );
            } else {
              return (
      ...
      <td key={cellKey} className="centeredDiv">
        <div>
          <div className="multilineText">
            ...
            {email ? (
              <div style={{ lineHeight: "1em", fontSize: `${fontSize}` }}>
                {email}
              </div>
            ) : null}
          </div>
        </div>
        {!email && !phone && <div>No informado</div>}
      </td>
    );
  };
  ...
  return (
    <div className="div-table">
      <table className="table content table-responsive-md">
        <thead className="table-header">{renderTableHeaders(attributes)}</thead>
